...
これらのサンプルでイーサネットを使用したい場合は、単に `manifest_net.json` を `manifest_net_ethernet.json` に置き換えるだけです。`manifest_net.json` と `manifest_net_ethernet.json` の両方をインクルードすべきではないことに注意してください。どちらか一方のみをインクルードしてください。
...
> **注:** アプリケーションは、イーサネット接続を開始するために`Ethernet`クラスの静的メソッド`start`を呼び出す必要があります。`start`を呼び出さないと、`Ethernet`コンストラクタに渡されたコールバックは決して呼び出されません。
...
次の例は、イーサネットへの接続プロセスを開始し、接続が成功してイーサネットデバイスにIPアドレスが割り当てられたときにコンソールにログ出力します。
