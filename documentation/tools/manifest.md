...
#### `static` メモリ割り当て
`static` プロパティはマイクロコントローラにとって最も重要です。これは、スタック、オブジェクト、バイトコード、文字列など、JavaScript言語ランタイムが使用できるバイト数の合計です。ブックキーピングのオーバーヘッドを最小限に抑え、ランタイムが固定サイズのスロットと可変サイズのチャンクの領域を動的に管理できるように、単一のメモリブロックとして割り当てられます。`static` プロパティは、スクリプトがメモリ予算を超えることができないように（できれば、スクリプトがホストOSに必要なメモリを取ることができ、障害や不安定を引き起こす可能性があるため）、言語ランタイムによって割り当てられるメモリに厳格な制限を課します。

`static` プロパティはシミュレーターによって無視されます。シミュレーターはオンデマンドでメモリ割り当てにフォールバックします。コンピュータはマイクロコントローラに比べてほぼ無限のメモリを持っているため、これは問題ではありません。
...
- `static` プロパティを `0` に設定して、静的アロケータを無効にします。（プロパティを削除するだけでは不十分です。`static` プロパティは含まれているマニフェストで定義されている可能性があります）
...
また、各プラットフォームのファイルシステムのルートを指定するために [files manifest](../../modules/files/file/manifest.json)、いくつかのネットワーキングの例でWi-Fi認証情報を指定するためにも使用されます。
