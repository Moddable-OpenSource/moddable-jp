...
<a id="mcrun"></a>
## mcrun
**mcrun** は、[mods](../xs/mods.md) をビルドするコマンドラインツールです。modsは、新しい機能を追加したり、既存の挙動を変更したりするスクリプトで、ユーザーがIoT製品にインストールできます。modをビルドするための入力は、JavaScriptモジュール、アセット、データ、および設定で、これらはマニフェストで指定されます。出力は、JavaScriptバイトコードとリソースデータを含むXSアーカイブファイル（`.xsa` 拡張子）です。
...
**mcrez**はリソースマップにアセットを含むコマンドラインツールです。**mcrez**はアセット自体とそれにアクセスする方法を含むCコードを生成します。
